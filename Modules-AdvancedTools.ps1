...
function Global:Optimize-BootShutdown {
    ...
    Write-Host "`n[OK] Boot and shutdown optimized!" -ForegroundColor $Script:Colors.Success
    Wait-ForUser
}
...
function Global:Tweak-FileExplorer {
    ...
    Write-Host "`n[OK] File Explorer tweaked!" -ForegroundColor $Script:Colors.Success
    Wait-ForUser
}
...
function Global:Tweak-TaskbarStartMenu {
    ...
    Write-Host "`n[OK] Taskbar and start menu tweaked!" -ForegroundColor $Script:Colors.Success
    Wait-ForUser
}
...
function Global:Configure-Notifications {
    ...
    Write-Host "`n[OK] Notifications configured!" -ForegroundColor $Script:Colors.Success
    Wait-ForUser
}
...
function Global:Optimize-WindowsDefender {
    ...
    Write-Host "`n[OK] Windows Defender optimized!" -ForegroundColor $Script:Colors.Success
...
function Global:Configure-FontRendering {
    ...
    Write-Host "`n[OK] Font rendering configured!" -ForegroundColor $Script:Colors.Success
    Wait-ForUser
}
...
function Global:Apply-RegistryTweaks {
    ...
    Write-Host "`n[OK] Registry tweaks applied!" -ForegroundColor $Script:Colors.Success
    Wait-ForUser
}
...
function Global:Apply-AllAdvancedTweaks {
    ...
    Write-Host "`n[OK] ALL ADVANCED TWEAKS COMPLETED!" -ForegroundColor $Script:Colors.Success
    Wait-ForUser
}
...
function Global:Optimize-Presentation {
    ...
    Write-Host "`n[OK] Presentation / Battery mode applied!" -ForegroundColor $Script:Colors.Success
...
function Global:Show-SystemInfoDashboard {
    ...
    Write-Host "  =======================================================================" -ForegroundColor $Script:Colors.Menu
    Write-Host ""
    Write-Host "  [PC] CPU:       $($sysInfo.CPU) [$($hw.CPU.Vendor)]" -ForegroundColor White
    Write-Host "  [CPU] RAM:       $($sysInfo.RAM)" -ForegroundColor White
    if ($hw.GPUs.Count -eq 0) {
        Write-Host "  [GAME] GPU:       Not detected" -ForegroundColor White
    }
    else {
        foreach ($gpu in $hw.GPUs) {
            Write-Host "  [GAME] GPU:       $($gpu.Name) [$($gpu.Vendor)]$(if ($gpu.IsActive) { ' (active)' })" -ForegroundColor White
        }
    }
    Write-Host "  [WIN] OS:        $($sysInfo.OS)" -ForegroundColor White
    Write-Host "  [TIME]  Uptime:    $($sysInfo.Uptime.Days)d $($sysInfo.Uptime.Hours)h $($sysInfo.Uptime.Minutes)m" -ForegroundColor White
    Write-Host "  [TARGET] Platform:  $(Show-HardwarePlatformSummary)" -ForegroundColor Magenta
    Write-Host ""
    ...
    foreach ($drive in $drives) {
        ...
        Write-Host "  [DISK] Drive $($drive.DriveLetter):  $freeGB GB free / $totalGB GB total ($usedPercent% used)" -ForegroundColor White
    }
    Write-Host ""
...
function Global:Run-PerformanceBenchmark {
    ...
    Write-Host "`n[OK] Benchmark complete!" -ForegroundColor $Script:Colors.Success
    Wait-ForUser
}
...
function Global:Run-SystemHealthCheck {
    ...
    Write-Host "`n[OK] System health check complete!" -ForegroundColor $Script:Colors.Success
...
function Global:Create-SystemRestorePoint {
    ...
    try {
        ...
        Write-Host "`n[OK] System restore point created!" -ForegroundColor $Script:Colors.Success
        ...
        Write-Host "`n[X] Failed to create system restore point." -ForegroundColor $Script:Colors.Error
...
function Global:Backup-CurrentSettings {
    ...
    try {
        if ($Script:ConfigBackup.Count -gt 0) {
            ...
            Write-Host "`n[OK] Settings backed up!" -ForegroundColor $Script:Colors.Success
        ...
        Write-Host "`n[X] Failed to create backup." -ForegroundColor $Script:Colors.Error
...
function Global:Restore-PreviousSettings {
    ...
    try {
        ...
        Write-Host "`n[OK] Settings restored from $($selectedFile.Name)!" -ForegroundColor $Script:Colors.Success
        ...
        Write-Host "`n[X] Failed to restore backup: $($_.Exception.Message)" -ForegroundColor $Script:Colors.Error
...
function Global:Undo-LastOptimization {
    ...
    try {
        ...
        Write-Host "`n[OK] Reverted: $($lastAction.Description)" -ForegroundColor $Script:Colors.Success
        Write-Log "Undid last optimization: $($lastAction.Description)" -Level Success -Category "Undo"
    }
    catch {
        Write-Host "`n[X] Failed to undo: $($_.Exception.Message)" -ForegroundColor $Script:Colors.Error
...
function Global:Export-Configuration {
    ...
    try {
        ...
        Write-Host "`n[OK] Configuration exported!" -ForegroundColor $Script:Colors.Success
        ...
        Write-Host "`n[X] Failed to export configuration: $($_.Exception.Message)" -ForegroundColor $Script:Colors.Error
